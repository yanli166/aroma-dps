...
INPUT_FOLDER2="/home/ubuntu/cal/DPSCAL/lunci3/homatxt"
OUTPUT_FOLDER="/home/ubuntu/cal/DPSCAL/lunci3/OUTHOMA"
# 遍历文件夹中的所有.wfn文件
for wfn_file in "$INPUT_FOLDER"/*.fchk; do
    ...
    for ring_id in {1..10}; do
        # 构建对应的homa-*-*.txt文件名
        homa_filename="homa-$filename-ring$ring_id.txt"
        input_txt="$INPUT_FOLDER2/$homa_filename"
        if [ ! -f "$input_txt" ]; then
