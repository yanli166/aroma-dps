"""
GNN - 基础图卷积网络 (Kipf & Welling GCN-style)
...
class ConvolutionLayer(nn.Module):
    ...
    def forward(self, x, adj):
        ...
        adj_norm = adj * d_inv_sqrt * d_inv_sqrt.transpose(1, 2)
        agg = torch.matmul(adj_norm, x)
        return self.linear(agg)


class GNNModel(nn.Module):
    """GNN主模型，统一支持label/mask/pool三种模式"""
    def __init__(self, node_vec_len, hidden_dim, n_conv, n_hidden, n_outputs,
        ...
        self.conv_layers = nn.ModuleList([
            ConvolutionLayer(hidden_dim, hidden_dim)
