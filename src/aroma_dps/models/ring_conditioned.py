...
复用 vendored 的 unified_models 卷积层实现 (models/backbones/), 仅替换 readout。

约束: 最终模型用 binary membership (0/1) + nn.Embedding 可学习投影 (Stage5/6 结论);
但 Fig.3 的 2x2 消融必须保留 ring_flag=10 的原始因子定义, 两者不得统一。

Vendored from 0831-end-code/models/ring_conditioned_gnn.py (logic unchanged;
only the backbone/readout imports were made package-relative).
"""
import torch
