...
对 Layer 1/2/3 用多个随机种子重复实验, 求平均 ± 标准差, 提升结果稳健性。
不修改原有代码, 仅做薄层包装, 复用现有训练函数。
...
def _load_completed_seeds(output_root):
    """从 checkpoint 文件加载已完成的种子 (断点续跑, 只信任完整跑完的)"""
    csv_path = os.path.join(output_root, 'per_seed_results.csv')
    results = []
    ...
    # 如果没有 checkpoint, 也检查 per_seed_results.csv (兼容旧格式)
...
def run_layer1(seeds, tasks, gpu, output_root):
    """多种子 Layer 1 ML 实验"""
    from baseline_traditional_ml.code.ml_train_eval import build_models, run_one_model
    ...
    for seed in seeds:
        ...
        for task in task_list:
            ...
            splits = canonical_splits(len(y), seed=seed)
            models = build_models(seed=seed, gpu=gpu)
            for model_name, model in models.items():
                factory = (lambda m=model: type(m)(**m.get_params()))
                try:
                    res = run_one_model(model_name, factory, X, y, splits, name, seed_dir)
                    res['seed'] = seed
        ...
        print(f"  [Seed={seed}] 完成, 已增量保存到 per_seed_results.csv", flush=True)
...
def aggregate_results(output_root, layer):
    ...
    # 聚合列
    metric_cols = ['cv_r2', 'cv_r2_std', 'cv_mae', 'cv_rmse',
                   'test_r2', 'test_mae', 'test_rmse']
    if 'train_r2' in df.columns:
        metric_cols = ['train_r2', 'train_mae', 'train_rmse'] + metric_cols
    ...
    display_cols = [c for c in ['task', 'model', 'encoding', 'test_r2_mean', 'test_r2_std',
                                'test_mae_mean', 'test_rmse_mean'] if c in agg.columns]
    print(agg[display_cols].to_string(index=False))
    return agg


def recover_from_summaries(output_root, layer):
    """从 individual summary.csv 文件重建 per_seed_results.csv (进程崩溃恢复)"""
    import glob
    pattern = os.path.join(output_root, 'seed_*', '*', '*', 'summary.csv')
    files = glob.glob(pattern)
    if not files:
        print(f"未找到 summary.csv 文件 in {output_root}")
        return None

    results = []
    for f in files:
        try:
            # 路径: output_root/seed_X/TASK/MODEL[/ENCODING]/summary.csv
            ...
            if 'layer3' in layer or 'ring' in layer:
                # model_encoding 格式 (如 GAT_label, GraphSAGE_pool)
            ...
            for k in ['n', 'cv_r2', 'cv_r2_std', 'cv_mae', 'cv_rmse',
...
def _merge_parts_and_aggregate():
    """合并 part 目录的 per_seed_results.csv, 然后聚合所有层"""
    results_root = os.path.join(PROJ_ROOT, 'new5zhecv', 'results')

    # Layer 2: 合并 part1/2/3 -> layer2_gnn
    layer2_main = os.path.join(results_root, 'layer2_gnn')
    layer2_parts = ['layer2_gnn_part1', 'layer2_gnn_part2', 'layer2_gnn_part3']
    _merge_parts(results_root, layer2_main, layer2_parts, 'layer2_gnn')

    # Layer 3: 合并 part1/2/3 -> layer3_ring
    layer3_main = os.path.join(results_root, 'layer3_ring')
    layer3_parts = ['layer3_ring_part1', 'layer3_ring_part2', 'layer3_ring_part3']
...
def _merge_parts(results_root, main_dir, part_names, layer_name):
    ...
    for part in part_names:
        ...
        # 优先用 per_seed_results.csv, 没有则从 summary.csv 恢复
    ...
    if all_dfs:
        ...
        print(f"  -> 合并 {len(merged)} 行到 {main_csv}")
    else:
        print(f"  -> 无可合并数据")
...
# ============== 主入口 ==============
def main():
    parser = argparse.ArgumentParser(description='多种子 × 5折CV 实验')
    parser.add_argument('--layer', type=int, choices=[1, 2, 3], default=None,
                        help='实验层: 1=ML, 2=GNN, 3=Ring')
    ...
    parser.add_argument('--recover', action='store_true',
                        help='从 individual summary.csv 重建 per_seed_results.csv')
    ...
    if args.recover:
        # 从 individual summary.csv 恢复
        layer_map = {1: 'layer1_ml', 2: 'layer2_gnn', 3: 'layer3_ring'}
        if args.layer:
            dirs = [args.output_root or os.path.join(PROJ_ROOT, 'new5zhecv', 'results', layer_map[args.layer])]
        else:
            # 恢复所有 part 目录
            import glob
            dirs = glob.glob(os.path.join(PROJ_ROOT, 'new5zhecv', 'results', 'layer*'))
    ...
    output_root = args.output_root or os.path.join(PROJ_ROOT, 'new5zhecv', 'results', layer_name)
