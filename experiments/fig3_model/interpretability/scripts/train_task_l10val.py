# -*- coding: utf-8 -*-
"""
Step B (generic): NICS_1zz / MBCO 重训 — 训练=collet a/b + l10 80%, 验证/测试=l10 20%

Stage 6 最优配置 (NICS_1zz / MBCO: proj=False), seed/超参与 best_model_package 一致。
early stopping 直接基于 l10 验证集 (与 HOMA_l10val 相同协议)。

用法: CUDA_VISIBLE_DEVICES=2 python3 train_task_l10val.py --task NICS_1zz --gpu 0
输出: models/{task}_l10val_best.pt
      results/{task}_l10val_metrics.json
      results/{task}_l10val_per_ringfamily.csv
      predictions/{task}_l10val_predictions.csv
"""
import os
import sys
import json
...
TASKS = {"NICS_1zz": {"proj": False, "l10_label": "NICS_ZZ"},
         "MBCO": {"proj": False, "l10_label": "MBCO"}}
...
def main():
    ...
    args = ap.parse_args()
    task = args.task
    proj = TASKS[task]["proj"]
    l10_label = TASKS[task]["l10_label"]
    device = torch.device(f"cuda:{args.gpu}" if torch.cuda.is_available() else "cpu")
    print(f"[train {task} l10val] device={device} seed={MODEL_SEED} proj={proj}")

    df = pd.read_csv(os.path.join(DATA_DIR, f"{task.lower()}_l10val.csv"))
    ...
    print(f"\n[l10_val {task}] n={n_va} R2={r2:.4f} MAE={mae:.4f} RMSE={rmse:.4f} "
          f"Spearman={spr:.4f}")

    # 保存
    model_path = os.path.join(MODEL_DIR, f"{task}_l10val_best.pt")
    ...
    metrics = {"task": task, "val": "l10 20% (molecule-grouped, 复用 HOMA 分子划分)",
    ...
    with open(os.path.join(RESULT_DIR, f"{task}_l10val_metrics.json"), "w") as f:
        json.dump(metrics, f, indent=2, ensure_ascii=False)

    # 预测 CSV + l10 元数据 (ring_name/sub_name/sub_type/ring_pos)
    ...
    l10m = l10m[l10m[l10_label].notna()].reset_index(drop=True)
    ...
    for col in ["ring_name", "sub_name", "sub_type", "ring_pos"]:
        pred_df[col] = pred_df.apply(
            lambda r, c=col: lmap.get((str(r["smiles"]), str(sorted(r["atom_on_ring"]))),
                                      {}).get(c, ""), axis=1)
    pred_csv = os.path.join(PRED_DIR, f"{task}_l10val_predictions.csv")
    ...
    g.to_csv(os.path.join(RESULT_DIR, f"{task}_l10val_per_ringfamily.csv"))
    print(f"\n[saved] {model_path}")
    print(f"[saved] {os.path.join(RESULT_DIR, f'{task}_l10val_metrics.json')}")
    print(f"[saved] {os.path.join(RESULT_DIR, f'{task}_l10val_per_ringfamily.csv')}")
